...
export default function ScreenReplacement() {
  const pageUrl = `${BUSINESS_INFO.url}/screen-replacement`;
    ...
    "@graph": [
      {
        "@type": "WebPage",
        "@id": `${pageUrl}#webpage`,
        "name": "Screen Replacement Kuwait | Laptop & MacBook Screen Fix",
        "url": pageUrl,
      ...
      },
      {
        ...
        "hasOfferCatalog": {
          ...
          "itemListElement": [
            { "@type": "Offer", "position": 1, "itemOffered": { "@type": "Service", "name": "Laptop LCD/LED Screen Replacement" }, "priceSpecification": { "@type": "PriceSpecification", "price": "20", "priceCurrency": "KWD", "minPrice": "20" } },
            { "@type": "Offer", "position": 2, "itemOffered": { "@type": "Service", "name": "MacBook Retina Display Replacement" }, "priceSpecification": { "@type": "PriceSpecification", "price": "45", "priceCurrency": "KWD", "minPrice": "45" } }
          ]
        }
      },
      {
      ...
      },
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 3, "name": "Screen Replacement", "item": pageUrl }
        ]
      }
    ]
  }), [pageUrl]);
  ...
  return (
    <main className="w-full min-h-screen bg-transparent text-slate-200 selection:bg-cyan-500/30 pt-32 pb-24 font-sans">
      <Helmet>
        ...
        <meta name="description" content="Professional laptop and MacBook screen replacement in Kuwait. Cracked, broken, or flickering screens fixed fast with free pickup. Starting 20 KD. تبديل شاشة لاب توب الكويت." />
      ...
      <section className="relative px-6 text-center z-10 mb-24">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-white mb-6 tracking-tight leading-tight">
            Screen Replacement <br className="hidden md:block" />
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mb-8">
            <a href={waLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full sm:w-auto bg-cyan-500 hover:bg-cyan-400 text-slate-950 px-8 py-4 rounded-full font-black text-base transition-all shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:scale-[1.02]">
              Book Free Pickup <ExternalLink size={20} />
            </a>
          ...
          <div className="flex flex-wrap gap-2 justify-center max-w-3xl mx-auto mt-8">
            {AREAS.map((area, idx) => (
              <span key={idx} className="bg-slate-900/50 border border-slate-800 text-slate-400 text-xs px-3 py-1.5 rounded-full cursor-default">
                {area}
              </span>
            ))}
          </div>
      ...
      <section className="max-w-6xl mx-auto px-6 relative z-10 mb-24">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-black text-white mb-4 tracking-tight">Professional Screen Solutions</h2>
          <p className="text-slate-400">Fast, flawless replacements for any display defect.</p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SERVICES.map((service, idx) => (
            <div key={idx} className="bg-slate-900/30 backdrop-blur-md p-8 rounded-3xl border border-slate-800 hover:border-cyan-500/30 transition-colors h-full flex flex-col">
        ...
        <div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {WHY_CHOOSE_US.map((item, idx) => (
              <div key={idx} className="bg-slate-900/30 backdrop-blur-md border border-slate-800 rounded-2xl p-5 hover:border-cyan-500/30 transition-all flex items-start gap-4">
          ...
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {BRANDS.map((brand, idx) => (
                <span key={idx} className="bg-slate-900/50 border border-slate-800 text-slate-400 text-xs px-3 py-1.5 rounded-full cursor-default hover:text-cyan-400 hover:border-cyan-500/30 transition-colors">
                  {brand}
                </span>
              ))}
            </div>
      ...
      <section className="max-w-4xl mx-auto px-6 relative z-10">
        <div className="bg-gradient-to-br from-cyan-900/40 to-slate-900/80 backdrop-blur-xl p-10 md:p-16 rounded-3xl border border-cyan-500/30 text-center shadow-[0_0_40px_rgba(34,211,238,0.1)]">
          <MonitorPlay className="w-12 h-12 text-cyan-400 mx-auto mb-4" />
          <h2 className="text-3xl md:text-5xl font-black text-white mb-6 tracking-tight">Ready to Fix Your Screen?</h2>
