...
export default function LaptopRepair() {
  ...
  const COMMON_ISSUES = [
    { title: "Thermal Throttling", desc: "Fans running loud? Laptops overheating in Kuwait's climate will damage the CPU. We apply premium liquid metal and thermal paste.", icon: ThermometerSnowflake },
    { title: "Power & Battery Failures", desc: "Laptop dying at 30% or won't turn on at all? We replace degraded lithium cells and repair faulty charging ports.", icon: BatteryWarning },
    ...
    <main className="w-full min-h-screen bg-transparent text-slate-200 selection:bg-cyan-500/30 pt-32 pb-24">
      ...
      <section className="relative px-6 text-center z-10 mb-24">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-sm font-bold text-emerald-400 uppercase tracking-widest">
            <span className="flex items-center gap-2"><CheckCircle size={16}/> Dell</span>
            <span className="flex items-center gap-2"><CheckCircle size={16}/> HP</span>
            <span className="flex items-center gap-2"><CheckCircle size={16}/> Lenovo</span>
            <span className="flex items-center gap-2"><CheckCircle size={16}/> ASUS</span>
            <span className="flex items-center gap-2"><CheckCircle size={16}/> Acer</span>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 relative z-10 mb-24">
        <div className="grid md:grid-cols-3 gap-6">
          {COMMON_ISSUES.map((issue, idx) => (
            <div key={idx} className="bg-slate-900/30 backdrop-blur-md p-8 rounded-3xl border border-slate-800 hover:border-cyan-500/30 transition-colors group">
      ...
      <section className="max-w-4xl mx-auto px-6 relative z-10 mb-24">
        <div className="bg-slate-900/50 backdrop-blur-xl p-10 md:p-14 rounded-3xl border border-slate-800">
          <h2 className="text-3xl font-black text-white mb-8 text-center tracking-tight">Our Repair Protocol</h2>
          <div className="space-y-8">
            <div className="flex gap-6">
              <div className="w-10 h-10 shrink-0 bg-cyan-500/20 border border-cyan-500/50 rounded-full flex items-center justify-center text-cyan-400 font-black text-lg">1</div>
              <div>
                <h4 className="text-xl font-bold text-white mb-2">Free Collection & Diagnosis</h4>
                <p className="text-slate-400 text-sm leading-relaxed">Our driver collects your laptop. We perform a full teardown and component-level diagnostic test at absolutely no cost to you.</p>
              </div>
            </div>
            <div className="flex gap-6">
              <div className="w-10 h-10 shrink-0 bg-cyan-500/20 border border-cyan-500/50 rounded-full flex items-center justify-center text-cyan-400 font-black text-lg">2</div>
              <div>
                <h4 className="text-xl font-bold text-white mb-2">Transparent Quoting</h4>
                <p className="text-slate-400 text-sm leading-relaxed">We provide a firm price and turnaround time. We only proceed if you approve. If you decline, we return the laptop for free.</p>
              </div>
            </div>
            <div className="flex gap-6">
              <div className="w-10 h-10 shrink-0 bg-cyan-500/20 border border-cyan-500/50 rounded-full flex items-center justify-center text-cyan-400 font-black text-lg">3</div>
              <div>
                <h4 className="text-xl font-bold text-white mb-2">Precision Repair & Testing</h4>
                <p className="text-slate-400 text-sm leading-relaxed">We repair the fault, apply high-grade thermal paste, and stress-test the machine for 12 hours before returning it to you.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 relative z-10">
        <div className="bg-gradient-to-br from-cyan-900/40 to-slate-900/80 backdrop-blur-xl p-10 rounded-3xl border border-cyan-500/30 text-center shadow-[0_0_40px_rgba(34,211,238,0.1)]">
          ...
          <p className="text-slate-300 mb-8 max-w-xl mx-auto">Don't risk losing your data or causing further hardware damage. Let our experts diagnose it today.</p>
